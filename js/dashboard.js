...
async function addItemDispensation() {
    ...
    if (!medId) {
        showToast("Pesquise e selecione um medicamento na lista sugerida.");
        return;
    }
    ...
    if (!fefoResult.success) {
        showToast(`Estoque insuficiente! Temos apenas ${fefoResult.stock} unidades de ${med.activeIngredient} ${med.concentration}.`);
        return;
    }
    ...
    if (data.items && data.items.length > 0) {
        data.items.forEach(item => {
            ...
            tr.innerHTML = `
                ...
                <td class="text-center" style="width: 30%; text-align: center; font-family: var(--font-data); font-weight: 600; color: #0284c7; vertical-align: middle;">
                    ${escapeHTML(item.lotCode || '-')}
                </td>
                <td class="text-center" style="width: 30%; text-align: center; font-weight: 700; color: #ea580c; font-size: 15px; vertical-align: middle;">
                    ${item.quantity} un
                </td>
            `;
            body.appendChild(tr);
