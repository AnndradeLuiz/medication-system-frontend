...
window.initInventoryModule = function() {
    ...
    if (tbody && tbody.innerHTML.trim() === '') {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center p-40 text-muted">Defina seus filtros e clique em <b>Filtrar</b> (ou digite e aperte Enter) para carregar os dados.</td></tr>`;
    }
...
function renderInventoryPage(items) {
    ...
    if (!items || items.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align: center; padding: 40px; color: var(--color-text-muted);">Nenhum item encontrado com os filtros atuais.</td></tr>`;
        return;
    }
